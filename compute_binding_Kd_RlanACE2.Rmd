---
title: "Compute per-barcode RlanACE2 binding affinity"
author: "Tyler Starr"
date: "01/02/2023"
output:
  github_document:
...
This notebook reads in per-barcode counts from `count_variants.ipynb` for RlanACE2-binding Tite-seq experiments, computes functional scores for RBD ACE2-binding affiniity, and does some basic QC on variant binding functional scores.
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="lib40",Kd_RlanACE2]))` of our lib40 barcodes (`r round(sum(!is.na(dt[library=="lib40",Kd_RlanACE2]))/nrow(dt[library=="lib40",])*100,digits=2)`%) and (`r round(sum(!is.na(dt[library=="lib46",Kd_RlanACE2]))/nrow(dt[library=="lib46",])*100,digits=2)`%) of our lib46 pool. 
...
```{r Kd_distribution_lib46, fig.width=8, fig.height=4, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(1,2))
...
```{r binding_distribution_vioplot_lib46, echo=T, fig.width=18, fig.height=5, fig.align="center", dpi=300,dev="png"}
...
We have generated binding measurements for `r round(nrow(dt[!is.na(log10Ka_RlanACE2)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
