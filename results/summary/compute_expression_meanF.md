...
dt <- rbind(dt_Wh1,dt_RshSTT182,dt_PRD0038,dt_SARS1,dt_RsYN04,dt_RmYN02); setkey(dt,barcode,library)
...
dt <- merge(counts, dt, by=c("library","barcode")); rm(dt_Wh1);rm(dt_RshSTT182);rm(dt_PRD0038);rm(dt_SARS1);rm(dt_RsYN04);rm(dt_RmYN02);rm(counts); rm(duplicates)
```
...
```
    ...
    ## [1] "reads < cells for lib40 SortSeq_bin1 , un-normalized (ratio 0.636639 )"
    ## [1] "read:cell ratio for lib40 SortSeq_bin2 is 1.53464"
    ## [1] "read:cell ratio for lib40 SortSeq_bin3 is 1.57836379018613"
    ## [1] "read:cell ratio for lib40 SortSeq_bin4 is 1.41980631578947"
    ## [1] "reads < cells for lib41 SortSeq_bin1 , un-normalized (ratio 0.369496855345912 )"
    ## [1] "read:cell ratio for lib41 SortSeq_bin2 is 1.26238407079646"
    ## [1] "read:cell ratio for lib41 SortSeq_bin3 is 1.36006533333333"
    ## [1] "read:cell ratio for lib41 SortSeq_bin4 is 1.32947510822511"
...
invisible(dt[library=="lib40",c("expression","ML_sdF") := tryCatch(calc.MLmean(b1=SortSeq_bin1*20,b2=SortSeq_bin2*20,
                                                                      b3=SortSeq_bin3*20,b4=SortSeq_bin4*20,
                                                                      min.b1=log(20),min.b2=log(660.5),min.b3=log(18103.5),
                                                                      min.b4=log(45036.5),max.b4=log(229000)),
                                                          error=function(e){return(list(as.numeric(NA),as.numeric(NA)))}),by=c("library","barcode")])
invisible(dt[library=="lib41",c("expression","ML_sdF") := tryCatch(calc.MLmean(b1=SortSeq_bin1*20,b2=SortSeq_bin2*20,
                                                                      b3=SortSeq_bin3*20,b4=SortSeq_bin4*20,
                                                                      min.b1=log(20),min.b2=log(553.5),min.b3=log(12530.5),
                                                                      min.b4=log(33141.5),max.b4=log(229000)),
                                                          error=function(e){return(list(as.numeric(NA),as.numeric(NA)))}),by=c("library","barcode")])
...
#histograms
par(mfrow=c(2,2))
hist(log10(dt[library=="lib40",expr_count]+0.1),xlab="cell count (log10, plus 0.1 pseudocount)",main="lib40, all bc",col="gray50")
hist(log10(dt[library=="lib41",expr_count]+0.1),xlab="cell count (log10, plus 0.1 pseudocount)",main="lib41, all bc",col="gray50")
hist(log10(dt[library=="lib40" & !is.na(expression),expr_count]+0.1),xlab="cell count (log10, plus 0.1 pseudocount)",main="lib40, determined",col="gray50")
hist(log10(dt[library=="lib41" & !is.na(expression),expr_count]+0.1),xlab="cell count (log10, plus 0.1 pseudocount)",main="lib41, determined",col="gray50")
```
...
We have generated expression measurements for 88.04% of the barcodes in
our libraries.
