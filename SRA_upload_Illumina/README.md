...
The files are uploaded to a recently created BioProject [PRJNA962117](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA962117), which is set up to store all deep mutational scanning and mutational antigenic profiling experiments that use the SARSr and pan-sarbecovirus libraries used here (libs 40, 41, 46)
...
For the sample name, provide a short name that describes the sample, such as `SARSr_RBD_DMS`.
...
We are adding to an existing BioProject, so enter [PRJNA962117](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA962117) as the *Existing BioProject* and hit `Continue`.
Finally, add a sample title, such as "Illumina barcode sequencing for SARSr RBD ACE2 and expression DMS."
Then hit `Continue`, make sure everything looks correct, then hit `Submit`.

After a brief bit of processing, the *BioSample* submission should show up, along with a sample accession: *SAMN34384823*.
...
We are adding to an existing BioProject, so enter [PRJNA962117](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA770094) as the *Existing BioProject* and hit `Continue`.
...
This YAML has two entries:
 ...
 - The `barcode_runs` file, which is a CSV in the format of the repo's master barcode run file at [../data/barcode_runs.csv](../data/barcode_runs.csv). But importantly, this file should **only** have the barcode runs for the samples of interest for this *BioSample*. If you have already made a subset repo (see [../subset_data](../subset_data)) that just has those samples of interest, then you can put the path to the master [../data/barcode_runs.csv](../data/barcode_runs.csv) file for the whole repo. Otherwise you want to create a CSV file that just holds the barcode runs of interest and place that file in the current folder, and point to it in the `upload_config.yaml` file
