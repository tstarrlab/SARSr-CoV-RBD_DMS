---
title: "Compute per-barcode RalcACE2 binding affinity"
author: "Tyler Starr"
date: "01/02/2023"
output:
  github_document:
...
This notebook reads in per-barcode counts from `count_variants.ipynb` for RalcACE2-binding Tite-seq experiments, computes functional scores for RBD ACE2-binding affiniity, and does some basic QC on variant binding functional scores.
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="lib40",Kd_RalcACE2]))` of our lib40 barcodes (`r round(sum(!is.na(dt[library=="lib40",Kd_RalcACE2]))/nrow(dt[library=="lib40",])*100,digits=2)`%) and (`r round(sum(!is.na(dt[library=="lib46",Kd_RalcACE2]))/nrow(dt[library=="lib46",])*100,digits=2)`%) of our lib46 pool. 
...
We have generated binding measurements for `r round(nrow(dt[!is.na(log10Ka_RalcACE2)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
