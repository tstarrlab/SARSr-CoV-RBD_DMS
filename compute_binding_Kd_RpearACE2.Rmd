---
title: "Compute per-barcode RpearACE2 binding affinity"
author: "Tyler Starr"
date: "01/02/2023"
output:
  github_document:
...
This notebook reads in per-barcode counts from `count_variants.ipynb` for RpearACE2-binding Sort-seq experiments, computes functional scores for RBD ACE2-binding affiniity, and does some basic QC on variant binding functional scores.
...
Distribution of MFI estimates for lib40 backgrounds, with wt/syn barcodes in purple:

```{r MFI_distribution, fig.width=8, fig.height=10, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(3,2))
...
We have generated binding measurements for `r round(nrow(dt[!is.na(MFI_RpearACE2)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
